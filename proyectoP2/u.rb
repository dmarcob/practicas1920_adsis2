	 ...
	 if File.file?(f + "/" +  modulo)
		...
		maquinas.each do |host|
		 Net::SSH.start(host, "a755232",:timeout=> 7) do |ssh|
				   ...
				   os = ssh.exec!("uname")
				   if os.strip == "OpenBSD" #host tiene sistema operativo openBSD
                                      res = ssh.exec!("doas puppet module install /tmp/.puppet/" + modulo + " --force")
		    		   else #host tiene sistema operativo ubuntu/centOS
                                      res = ssh.exec!("sudo puppet module install /tmp/.puppet/" + modulo + " --force")
				   end
                                   ssh.exec!("rm -rf /tmp/.puppet")
                                   puts "máquina " + host + ": exito"
                                   puts res
...
options = ["p", "s", "c","m"] 	#Comandos disponibles
...
if command == "p"
	...
	modulo_puppet(maquinas, d_despliegue, ARGV[1])
end   
